...
PRIVATE void HandlePiCommsMessage_t(PiCommsMessage_t msg){
	SerialPrintln("HandlePiCommsMessage_t");
	if(msg.has_attachmentCommand){
		AAMod_AppendageCallback(msg.attachmentCommand);
	}
...
PRIVATE void PrintPiCommsMessage_t(PiCommsMessage_t msg){
	...
	if(msg.has_attachmentCommand){
		...
		SerialPrint("claw_state: %d\t", (msg.attachmentCommand.has_claw_state ? msg.attachmentCommand.has_claw_state : -1));
